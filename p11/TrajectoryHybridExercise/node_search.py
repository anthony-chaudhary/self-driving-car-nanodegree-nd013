...
heuristic = [[max(15-y, 15-x) for x in range(16)] for y in range(16)]
...
def search(grid, init, goal, debug_flag=False, info=False):
    ...
    print()
    goal_paths = 1   # Number of True goals to find.
    pass_through_init_attempts = 1  # Max times can pass through origin
    goal_counter = 0
    ...
    while True:
        ...
        if info is True: print("On path:", current_path.id, " cost:", m, "of ", len(paths.nodes),  "paths")

        costs = []
        last_node_in_path = True
        for i in current_path.nodes:
            if i.expanded is False:
                costs.append(i.g)
                last_node_in_path = False   # reset since condition is met
        ...
        # 3. Check if at start, or first node
            ...
            # Remove node from path if not valid actions and last_node
            # needs both conditions otherwise would prematurely end
            if l_ == 0 and last_node_in_path is True:
                current_path.expanded = True
...
best_path, paths = search(grid, init, goal, debug_flag=False, info=True)
#print("\nNodes:")
